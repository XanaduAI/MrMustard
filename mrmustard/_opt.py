...
class CircuitInterface(ABC):
    ...
    @abstractmethod
    def add_gate(self, gate) -> None: pass

    @abstractmethod
    def set_input(self, state) -> None: pass

    @abstractmethod
    def gaussian_output(self) -> State: pass

    @abstractmethod
    def fock_output(self, cutoffs:Sequence[int]) -> ArrayLike: pass
...
class OptimizerBackendInterface(ABC):
    ...
    @abstractmethod
    def _loss_and_gradients(self, symplectic_params, euclidean_params, loss_fn): pass

    @abstractmethod
    def _update_symplectic(self, symplectic_grads, symplectic_params) -> None: pass

    @abstractmethod
    def _update_euclidean(self, euclidean_grads, euclidean_params) -> None: pass

    @abstractmethod
    def _all_symplectic_parameters(self, circuits) -> List: pass
...
class BaseOptimizer(OptimizerBackendInterface):
    ...
    def minimize(self, circuit:Union[Sequence[CircuitInterface], CircuitInterface], loss_fn:Callable, max_steps:int=1000) -> Union[Sequence[CircuitInterface], CircuitInterface]:
        ...
        with bar:
            while not self.should_stop(max_steps):
                loss, symp_grads, eucl_grads = self._loss_and_gradients(symplectic_parameters, euclidean_parameters, loss_fn)
