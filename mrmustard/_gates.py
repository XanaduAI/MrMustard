...
class Gate(GateInterface):
    ...
    def __call__(self, state: State) -> State:
        return self._apply_gaussian_channel(
            state, self.modes, self.symplectic_matrix, self.displacement_vector, self.noise_matrix
        )
...
class Dgate(Gate):
    ...
    def __init__(
        ...
        self._repr_string: Callable[
            ...
            lambda x, y: f"modes={modes}, x={x}, x_bounds={x_bounds}, x_trainable={x_trainable}, y={y}, y_bounds={y_bounds}, y_trainable={y_trainable}, hbar={hbar}"
        )
        self.modes = modes
        self.mixing = False
        self.hbar = hbar
