...
class Optimizer:
    ...
    def minimize(
        ...
        """
        params = {
            kind: extract_parameters(by_optimizing, kind)
            for kind in ("symplectic", "orthogonal", "euclidean")
        }
    ...
    def should_stop(self, max_steps: int) -> bool:
        ...
        if len(self.loss_history) > 20:  # if loss varies less than 10e-6 over 20 steps
            if (
                sum(abs(self.loss_history[-i - 1] - self.loss_history[-i]) for i in range(1, 20))
                < 1e-6
            ):
