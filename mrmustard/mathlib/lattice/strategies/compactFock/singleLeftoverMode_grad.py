...
from mrmustard.mathlib.lattice.strategies.compactFock.helperFunctions import (
...
@njit(cache=True)
def calc_dA_dB(  # pragma: no cover
    m,
    n,
    i,
    arr_read_pivot,
...
@njit(cache=True)
def write_block_grad(  # pragma: no cover
    i,
    write,
    arr_read_pivot,
    read_GB,
...
@njit(cache=True)
def read_block(  # pragma: no cover
    arr_write,
    arr_write_dA,
    arr_write_dB,
    idx_write,
...
@njit(cache=True)
def use_diag_pivot_grad(  # pragma: no cover
    A,
    B,
    M,
    cutoff_leftoverMode,
...
@njit(cache=True)
def fock_representation_1leftoverMode_grad_NUMBA(  # pragma: no cover
    A,
    B,
    M,
    cutoff_leftoverMode,
