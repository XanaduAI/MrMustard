...
def clone_mode(cov: Matrix, means: Vector, modes: Union[int, Sequence[int]], times: int) -> Tuple[Matrix, Vector]:
    r"""
    Clones the modes of a covariance matrix and a means vector a given numbe of times
    and joins them to the state as new modes.
    Arguments:
        cov (Matrix): covariance matrix of the state being cloned
        means (Vector): means vector of the state being cloned
        modes (int or Sequence[int]): modes to clone
        times (int): number of times to clone
    Returns:
        Tuple[Matrix, Vector]: the covariance and means of the state with the additional cloned mode
    """
