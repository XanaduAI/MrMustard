...
class Converter():
    ...
    def _wigner_to_husimi(self, cov, means):
    ...
    def _cayley(self, X, c):
        r"""Returns the self._cayley transform of a matrix:
        ...
        Returns:
            Tensor: the self._cayley transform of X
        """
        I = math.eye(X.shape[0], dtype=X.dtype)
        return math.solve(X + c * I, X - c * I)


    def _wignerdm_to_bargmanndm(self, wignerdm: WignerDM) -> BargmannDM:
        ...
        A = math.matmul(
            self._cayley(self._pq_to_aadag(wignerdm.data.cov), c=0.5), math.Xmat(N)
        )  # X on the right, so the index order will be rho_{left,right}:
        Q, beta = self._wigner_to_husimi(wignerdm.data.cov, wignerdm.data.means)
    ...
    def _wignerket_to_bargmannket(self, wignerket: WignerKet) -> BargmannKet:
        r"""Converts the wigner representation in terms of covariance matrix and mean vector into the Bargmann A,B,C triple
    ...
    #DO NOT TOUCH FOR NOW, THIS IS FOR TRANSFORMATION.
    def _wigner_to_fock_U(self, X, d, shape):
    ...
    #DO NOT TOUCH FOR NOW, THIS IS FOR TRANSFORMATION.
    def _wigner_to_fock_Choi(self, X, Y, d, shape):
    ...
    @tensor_int_cache
    def oscillator_eigenstates(self, q: RealVector, cutoff: int) -> Tensor:
    ...
    def _fockket_to_wavefunctionqket(self, fockket: FockKet, qs: Optional[Sequence[Sequence[float]]] = None) -> WavefunctionQKet:
    ...
    def _fockdm_to_wavefunctionqdm(self, fockdm: FockDM, qs: Optional[Sequence[Sequence[float]]] = None) -> WavaefunctionQDM:
