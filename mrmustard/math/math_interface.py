...
class MathInterface(ABC):
    ...
    def random_symplectic(self, num_modes: int, max_r: float = 1.0) -> Tensor:
        ...
        """
        if num_modes == 1:
            W = np.exp(1j * settings.random_state.uniform(size=(1, 1)))
            V = np.exp(1j * settings.random_state.uniform(size=(1, 1)))
        else:
            W = unitary_group.rvs(dim=num_modes, random_state=settings.random_state)
            V = unitary_group.rvs(dim=num_modes, random_state=settings.random_state)
        r = settings.random_state.uniform(low=0.0, high=max_r, size=num_modes)
    ...
    def random_orthogonal(N: int) -> Tensor:
        ...
        if N == 1:
            return np.array([[1.0]])
        return ortho_group.rvs(dim=N, random_state=settings.random_state)

    def random_unitary(self, N: int) -> Tensor:
        """a random unitary matrix in :math:`U(N)`"""
        if N == 1:
            return self.exp(1j * settings.random_state.uniform(size=(1, 1)))
        return unitary_group.rvs(dim=N, random_state=settings.random_state)
