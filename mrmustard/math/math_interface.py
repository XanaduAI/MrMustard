...
class MathInterface(ABC):
    ...
    def sparse_matvec(self, matrix: Tensor, vector: Tensor, m_modes: List[int], v_modes: List[int], like_0:bool) -> Tensor:
        ...
        Returns:
            array: :math:`2M` or :math:`2N` new vector (depending on `like_0`)
        """
        ...

    @abstractmethod
    def sparse_matmul(self, matrix1: Tensor, matrix2: Tensor, m1_modes: List[int], m2_modes: List[int], m1like_0:bool, m2like_0:bool) -> Tensor:
        r"""Mode-wise matrix multiplication of a phase space matrix and a phase space matrix.
        Assumes inputs are in xxpp ordering.
