...
def fock_representation_compact_NUMBA(A, B, G0, M, cutoff, cutoff_leftoverMode, PARTITIONS, shape0, shape2, shape11,
    ...
    # resG = np.zeros(shape_leftoverMode+shape0_tuple,dtype=np.complex128)
    # for m in range(arr0.shape[0]):
    #     for n in range(arr0.shape[1]):
    #         resG[m,n] = arr0[m,n].reshape(shape0_tuple)

    # assert False # probably need to loop over A and B indices before reshaping
    ...
    # return resG[:,:,0,0],arr0_dA.reshape(shape_leftoverMode+shape0_tuple+A.shape)[:,:,0,0],arr0_dB.reshape(shape_leftoverMode+shape0_tuple+B.shape)[:,:,0,0]
