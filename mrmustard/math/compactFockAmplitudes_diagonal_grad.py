...
def use_offDiag_pivot_grad(A, B, M, cutoffs, params, d, arr0, arr2, arr1010, arr1001, arr1, arr0_dA, arr2_dA,
                           arr1010_dA, arr1001_dA, arr1_dA, arr0_dB, arr2_dB, arr1010_dB, arr1001_dB, arr1_dB):
    '''
    ...
    arr0_dA[params_adapted], arr0_dB[params_adapted] = calc_dA_dB(2 * d + 1, G_in_dA, G_in_dB, G_in, A, B, K_l, K_i, M,
                                                                  pivot_val, pivot_val_dA, pivot_val_dB)

    # Array2
    if params[d] + 2 < cutoffs[d]:
        arr2_dA[d][params], arr2_dB[d][params] = calc_dA_dB(2 * d, G_in_dA, G_in_dB, G_in, A, B, K_l, K_i, M, pivot_val,
                                                            pivot_val_dA, pivot_val_dB)

    # Array11
    for i in range(d + 1, M):
        if params[i] + 1 < cutoffs[i]:
            arr1010_dA[d][i - d - 1][params], arr1010_dB[d][i - d - 1][params] = calc_dA_dB(2 * i, G_in_dA, G_in_dB,
                                                                                            G_in, A, B, K_l, K_i, M,
                                                                                            pivot_val, pivot_val_dA,
                                                                                            pivot_val_dB)
            arr1001_dA[d][i - d - 1][params], arr1001_dB[d][i - d - 1][params] = calc_dA_dB(2 * i + 1, G_in_dA, G_in_dB,
                                                                                            G_in, A, B, K_l, K_i, M,
                                                                                            pivot_val, pivot_val_dA,
                                                                                            pivot_val_dB)
    ...
    for sum_params in range(sum(cutoffs)):
        for params in dict_params[sum_params]:
            ...
            for d in range(M):  # for over pivot off-diagonals
                if np.all(np.array(params)[:d] == 0) and (params[d] < cutoffs[
