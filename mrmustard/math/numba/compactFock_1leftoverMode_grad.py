...
def write_block_grad(i, write, arr_read_pivot, read_GB, G_in, A, B, K_i, K_l, cutoff_leftoverMode, arr_write_dA,
    ...
    K_l_adapted = K_l
    arr_write_dA[(0, 0) + write], arr_write_dB[(0, 0) + write] = calc_dA_dB(m, n, i, arr_read_pivot, read_GB,
                                                                            G_in_adapted, A_adapted, B, K_i,
                                                                            K_l_adapted, arr_read_pivot_dA,
    ...
    for n in range(1, cutoff_leftoverMode):
        ...
        arr_write_dA[(0, n) + write], arr_write_dB[(0, n) + write] = calc_dA_dB(m, n, i, arr_read_pivot, read_GB,
                                                                                G_in_adapted, A_adapted, B, K_i,
                                                                                K_l_adapted, arr_read_pivot_dA,
                                                                                G_in_dA_adapted, arr_read_pivot_dB,
                                                                                G_in_dB_adapted, l_range)
    ...
    for m in range(1, cutoff_leftoverMode):
        ...
        arr_write_dA[(m, 0) + write], arr_write_dB[(m, 0) + write] = calc_dA_dB(m, n, i, arr_read_pivot, read_GB,
                                                                                G_in_adapted, A_adapted, B, K_i,
                                                                                K_l_adapted, arr_read_pivot_dA,
                                                                                G_in_dA_adapted, arr_read_pivot_dB,
                                                                                G_in_dB_adapted, l_range)
...
def use_offDiag_pivot_grad(A,B,M,cutoff_leftoverMode,cutoffs_tail,params,d,submatrices,submatrices_dA,submatrices_dB):
    ...
    arr0_dA, arr0_dB = write_block_grad(2 * d + 3, write, arr1, read_GB, G_in, A, B, K_i, K_l, cutoff_leftoverMode,
                                        arr0_dA, arr1_dA, G_in_dA, arr0_dB, arr1_dB, G_in_dB)

    # Array2
    if params[d] + 2 < cutoffs_tail[d]:
        write = (d,) + params
        arr2_dA, arr2_dB = write_block_grad(2 * d + 2, write, arr1, read_GB, G_in, A, B, K_i, K_l, cutoff_leftoverMode,
                                            arr2_dA, arr1_dA, G_in_dA, arr2_dB, arr1_dB, G_in_dB)

    # Array11
    for i in range(d + 1, M):
        if params[i] + 1 < cutoffs_tail[i]:
            write = (d, i - d - 1) + params
            arr1010_dA, arr1010_dB = write_block_grad(2 * i + 2, write, arr1, read_GB, G_in, A, B, K_i, K_l,
                                                      cutoff_leftoverMode, arr1010_dA, arr1_dA, G_in_dA, arr1010_dB,
                                                      arr1_dB, G_in_dB)
            arr1001_dA, arr1001_dB = write_block_grad(2 * i + 3, write, arr1, read_GB, G_in, A, B, K_i, K_l,
                                                      cutoff_leftoverMode, arr1001_dA, arr1_dA, G_in_dA, arr1001_dB,
                                                      arr1_dB, G_in_dB)

    return arr0_dA, arr2_dA, arr1010_dA, arr1001_dA, arr0_dB, arr2_dB, arr1010_dB, arr1001_dB

@njit
def use_diag_pivot_grad(A, B, M, cutoff_leftoverMode, cutoffs_tail, params, arr0, arr1, arr0_dA, arr1_dA, arr0_dB,
                        arr1_dB):
    '''
        ...
        if params[i // 2] + 1 < cutoffs_tail[i // 2]:
            ...
            if i != 1 or params[0] + 2 < cutoffs_tail[0]:
                ...
                arr1_dA, arr1_dB = write_block_grad(i + 2, write, arr0, read_GB, G_in, A, B, K_i, K_l, cutoff_leftoverMode,
                                                    arr1_dA, arr0_dA, G_in_dA, arr1_dB, arr0_dB, G_in_dB)

    return arr1_dA, arr1_dB

@njit
def fock_representation_1leftoverMode_grad_NUMBA(A, B, M, cutoff_leftoverMode, cutoffs_tail, arr0, arr2, arr1010,
    ...
    for m in range(cutoff_leftoverMode - 1):
        arr0_dA[(m + 1, 0) + zero_tuple] = (arr0_dA[(m, 0) + zero_tuple] * B[0] + np.sqrt(m) * A[0, 0] * arr0_dA[
            (m - 1, 0) + zero_tuple]) / np.sqrt(m + 1)
        arr0_dA[(m + 1, 0) + zero_tuple][0, 0] += (np.sqrt(m) * arr0[(m - 1, 0) + zero_tuple]) / np.sqrt(m + 1)
        arr0_dB[(m + 1, 0) + zero_tuple] = (arr0_dB[(m, 0) + zero_tuple] * B[0] + np.sqrt(m) * A[0, 0] * arr0_dB[
            (m - 1, 0) + zero_tuple]) / np.sqrt(m + 1)
        arr0_dB[(m + 1, 0) + zero_tuple][0] += arr0[(m, 0) + zero_tuple] / np.sqrt(m + 1)

    for m in range(cutoff_leftoverMode):
        for n in range(cutoff_leftoverMode - 1):
            arr0_dA[(m, n + 1) + zero_tuple] = (arr0_dA[(m, n) + zero_tuple] * B[1] + np.sqrt(m) * A[1, 0] * arr0_dA[
                (m - 1, n) + zero_tuple] + np.sqrt(n) * A[1, 1] * arr0_dA[(m, n - 1) + zero_tuple]) / np.sqrt(n + 1)
            arr0_dA[(m, n + 1) + zero_tuple][1, 0] += (np.sqrt(m) * arr0[(m - 1, n) + zero_tuple]) / np.sqrt(n + 1)
            arr0_dA[(m, n + 1) + zero_tuple][1, 1] += (np.sqrt(n) * arr0[(m, n - 1) + zero_tuple]) / np.sqrt(n + 1)
            arr0_dB[(m, n + 1) + zero_tuple] = (arr0_dB[(m, n) + zero_tuple] * B[1] + np.sqrt(m) * A[1, 0] * arr0_dB[
                (m - 1, n) + zero_tuple] + np.sqrt(n) * A[1, 1] * arr0_dB[(m, n - 1) + zero_tuple]) / np.sqrt(n + 1)
...
def fock_representation_1leftoverMode_grad(A, B, M, arr0, arr2, arr1010, arr1001, arr1):
    ...
    return fock_representation_1leftoverMode_grad_NUMBA(A, B, M, cutoff_leftoverMode, cutoffs_tail, arr0, arr2,
                                                        arr1010, arr1001, arr1, tuple_type, list_type, zero_tuple)
