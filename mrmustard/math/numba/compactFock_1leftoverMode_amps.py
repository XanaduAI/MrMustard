...
def use_offDiag_pivot(A, B, M, cutoff_leftoverMode, cutoffs_tail, params, d, arr0, arr2, arr1010, arr1001, arr1):
    ...
    if params[d] + 2 < cutoffs_tail[d]:
        write = (d,) + params
        arr2 = write_block(2 * d + 2, arr2, write, arr1, read_GB, G_in, GB, A, K_i, cutoff_leftoverMode)

    # Array11
    for i in range(d + 1, M):
        if params[i] + 1 < cutoffs_tail[i]:
            write = (d, i - d - 1) + params
            arr1010 = write_block(2 * i + 2, arr1010, write, arr1, read_GB, G_in, GB, A, K_i, cutoff_leftoverMode)
            arr1001 = write_block(2 * i + 3, arr1001, write, arr1, read_GB, G_in, GB, A, K_i, cutoff_leftoverMode)
...
def fock_representation_1leftoverMode_amps_NUMBA(A, B, M, cutoff_leftoverMode, cutoffs_tail, arr0, arr2, arr1010,
    ...
    for m in range(cutoff_leftoverMode - 1):
        arr0[(m + 1, 0) + zero_tuple] = (arr0[(m, 0) + zero_tuple] * B[0] + np.sqrt(m) * A[0, 0] * arr0[
            (m - 1, 0) + zero_tuple]) / np.sqrt(m + 1)
    for m in range(cutoff_leftoverMode):
        for n in range(cutoff_leftoverMode - 1):
            arr0[(m, n + 1) + zero_tuple] = (arr0[(m, n) + zero_tuple] * B[1] + np.sqrt(m) * A[1, 0] * arr0[
                (m - 1, n) + zero_tuple] + np.sqrt(n) * A[1, 1] * arr0[(m, n - 1) + zero_tuple]) / np.sqrt(n + 1)
