...
def test_matmul_all_different_modes(xpxp_matrix):
    ...
    matrix1 = np.block([[xpxp_matrix, np.zeros((2*N,2*N))], [np.zeros((2*N,2*N)), np.eye(2*N)]])  # add one extra empty mode at the end
    matrix2 = np.block([[np.eye(2*N), np.zeros((2*N,2*N))], [np.zeros((2*N,2*N)), xpxp_matrix]])  # add one extra empty mode at the beginning
    prod = xp1 @ xp2
    assert np.allclose(prod.to_xpxp(), matrix1 @ matrix2)
...
@given(square_matrix())
def test_addition_all_same_modes(xpxp_matrix):
    N = xpxp_matrix.shape[0]//2
    xp1 = XPTensor.from_xpxp(xpxp_matrix, modes=list(range(N)), like_1=True)
...
@given(square_matrix())
def test_addition_few_different_modes(xpxp_matrix):
    N = xpxp_matrix.shape[0]//2
    xp1 = XPTensor.from_xpxp(xpxp_matrix, modes=list(range(N)), like_1=True)
    xp2 = XPTensor.from_xpxp(xpxp_matrix, modes=list(range(1,N+1)), like_1=True)
