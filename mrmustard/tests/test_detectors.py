...
def test_homodyne_on_2mode_squeezed_vacuum_with_angle(s, X, angle):
    ...
    cov = tmsv.hbar/2*np.array([[1+2*s-2*s*(s+1)*(1+2*s+np.cosh(2*r)+np.cos(angle)*np.sinh(2*r))/denom,2*s*(1+s)*np.sin(angle)*np.sinh(2*r)/denom],
                                [2*s*(1+s)*np.sin(angle)*np.sinh(2*r)/denom, (1+2*s+(1+2*s*(1+s))*np.cosh(2*r)+2*s*(s+1)*np.cos(angle)*np.sinh(2*r))/denom]])
    assert np.allclose(remaining_state.cov, cov)
    denom = 1+2*s*(1+s)+(1+2*s)*np.cosh(2*r)
    means = np.array([np.sqrt(s*(1+s))*X*(np.cos(angle)*(1+2*s+np.cosh(2*r))+np.sinh(2*r))/denom,
    -np.sqrt(s*(1+s))*X*(np.sin(angle)*(1+2*s+np.cosh(2*r)))/denom]) * np.sqrt(2*tmsv.hbar)
    assert np.allclose(remaining_state.means, means)
...
def test_homodyne_on_2mode_squeezed_vacuum_with_displacement(s, X, d):
    ...
    prob, remaining_state = homodyne(tmsv)
    xb,xa,pb,pa = d
    means = np.array([xa+(2*np.sqrt(s*(s+1))*(X-xb))/(1+2*s+np.cosh(2*r)-np.sinh(2*r)),pa+(2*np.sqrt(s*(s+1))*pb)/(1+2*s+np.cosh(2*r)+np.sinh(2*r))]) * np.sqrt(2*tmsv.hbar)
    assert np.allclose(remaining_state.means, means)

# Test heterodyne now
@given(s= st.floats(min_value=0.0, max_value=10.0), x=st.floats(-10.0, 10.0), y=st.floats(-10.0, 10.0), d=arrays(np.float64, 4, elements=st.floats(-10.0, 10.0)))
