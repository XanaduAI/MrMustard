...
class Mixed(Representation):
    ...
    where the `z` can be mixed variable of real and complex variables acrossing all the different representations.

    This function allows for vector space operations on Mixed representation objects including
    linear combinations (``+``), outer product (``&``), and inner product (``@``) with a representation check [To explain more].

    Args:
        dict_rep_idxs: A dictionary that indicates each indices correspond to which representation.
        A: A batch of quadratic coefficient :math:`A_i`.
        b: A batch of linear coefficients :math:`b_i`.
        c: A batch of arrays :math:`c_i`.
    ...
    def conj(self):
        r"""
        The conjugate of this Mixed object.
        """
        new = self.__class__(math.conj(self.A), math.conj(self.b), math.conj(self.c))
        new._contract_idxs = self._contract_idxs  # pylint: disable=protected-access
        return new