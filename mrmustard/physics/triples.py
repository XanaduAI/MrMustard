...
def displacement_map_s_parametrized_Abc(s: int) -> Union[Matrix, Vector, Scalar]:
    ...
    the indices of the final triple correspond to the variables :math:`(\gamma^*, \gamma, z^*, z)` of the Bargmann function of the s-parametrized displacement map, and correspond to ``out_bra, out_ket, in_bra, in_ket`` wires.
    ...
    Returns:
        The ``(A, b, c)`` triple of the single-mode ``s``-parametrized dispalcement gate :math:`D_s(\gamma)`.
    """
    A = math.block(
        [
