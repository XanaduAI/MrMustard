...
class PNR(Detector):
    ...
    """
    def __init__(self, mode: int, conditional_probs=None, quantum_efficiency: float = 1.0, dark_count_prob: float = 0.0, max_input_photons: int = 50):
        super().__init__(mode)
        ...
        if conditional_probs is not None:
            ...
            self._stochastic_channel = self._detector_backend.convolve_probs_1d(condprob, [dark_prior, np.identity(condprob.shape[1])[0]])
