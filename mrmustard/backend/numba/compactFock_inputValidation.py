...
import numpy as np
from mrmustard.backend.numba.compactFock_diagonal_amps import fock_representation_diagonal_amps
from mrmustard.backend.numba.compactFock_diagonal_grad import fock_representation_diagonal_grad
from mrmustard.backend.numba.compactFock_1leftoverMode_amps import (
    fock_representation_1leftoverMode_amps,
)
from mrmustard.backend.numba.compactFock_1leftoverMode_grad import (
...
def hermite_multidimensional_diagonal(A, B, G0, cutoffs, rtol=1e-05, atol=1e-08):
    """
    Validation of user input for mrmustard.backend.tensorflow.hermite_renormalized_diagonal
    """
...
def grad_hermite_multidimensional_diagonal(A, B, G0, arr0, arr2, arr1010, arr1001, arr1):
    """
    Validation of user input for gradients of mrmustard.backend.tensorflow.hermite_renormalized_diagonal
    """
    if A.shape[0] != B.shape[0]:
        raise ValueError("The matrix A and vector B have incompatible dimensions")
    M = A.shape[0] // 2
...
def hermite_multidimensional_1leftoverMode(A, B, G0, cutoffs, rtol=1e-05, atol=1e-08):
    """
    Validation of user input for mrmustard.backend.tensorflow.hermite_renormalized_1leftoverMode
    """
    input_validation(A, atol=atol, rtol=rtol)
    if A.shape[0] != B.shape[0]:
        raise ValueError("The matrix A and vector B have incompatible dimensions")
...
def grad_hermite_multidimensional_1leftoverMode(A, B, G0, arr0, arr2, arr1010, arr1001, arr1):
    """
    Validation of user input for gradients of mrmustard.backend.tensorflow.hermite_renormalized_1leftoverMode
    """
    if A.shape[0] != B.shape[0]:
        raise ValueError("The matrix A and vector B have incompatible dimensions")
    M = A.shape[0] // 2
