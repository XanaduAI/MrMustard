...
class AmplitudeComputer {
  ...
  public:
    ...
    size_t level_size(size_t n) const {
        return binomial_coeff(modes_ + n - 1, modes_ - 1);
    }
    ...
    size_t get_rep_idx_in_level(size_t n, BigUInt rep) const {
        ...
        const auto iter =
            std::find(pivot_reps_.begin() + starting_indices_[n],
                      pivot_reps_.begin() + starting_indices_[n + 1], rep);
        return std::distance(pivot_reps_.begin(), iter);
    }

    std::vector<size_t> upper_indices_in_level(size_t n, BigUInt rep,
                                               size_t skip) const {
        assert(n == count_ones(rep));
        MultisetGenerator mgntr(modes_, n);
    ...
    std::vector<size_t> lower_indices_in_level(size_t n, BigUInt rep) const {
        ...
        for (size_t idx = 0; idx < pivot.size(); idx++) {
            if (pivot[idx] > 0) {
                ...
                indices.push_back(get_rep_idx_in_level(n - 1, rep_low));
                ++pivot[idx];
            }
        }
        return indices;
    ...
    double consume_one_pivot(
        const std::span<const std::complex<double>> &A,
        ...
        for (size_t up_idx = 0; up_idx < up.size(); up_idx++) {
            ...
            amplitude += b[up_idx] * G_curr_level[level_idx];
            for (size_t m = 0; m < pivot.size(); m++) {
                amplitude += sqrt(pivot[m]) * A[up_idx * modes_ + m] *
                             G_lower_level[lo[m]];
            }
            amplitude /= sqrt(pivot[up_idx] + 1);
            G_upper_level[up[up_idx]] = amplitude;
    ...
    double
    fill_next_level(const std::span<const std::complex<double>> &A,
                    const std::span<const std::complex<double>> &b,
                    const std::span<const std::complex<double>> &G_lower_level,
                    const std::span<const std::complex<double>> &G_curr_level,
        ...
        double norm_sqr = 0.0;
        for (size_t level_idx = 0; level_idx < level_size(curr_level);
             level_idx++) {
            norm_sqr += consume_one_pivot(A, b, G_lower_level, G_curr_level,
                                          G_upper_level, curr_level, level_idx);
        }
        return norm_sqr;
    }
};
