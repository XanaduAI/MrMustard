...
from mrmustard.math.lattice.strategies.vanilla import vanilla_numba, vanilla_b_batch_numba
...
def test_vanilla_stable_batched():
    ...
    with settings(STABLE_FOCK_CONVERSION=True):
        ...
        A, b, c = (
            math.asnumpy(A),
            math.asnumpy(math.atleast_2d(b)),
            math.asnumpy(c),
        )  # for tf backend
        batched = vanilla_b_batch_numba((4, 4), A, b, c)
        non_batched = vanilla_numba((4, 4), A, b[0], c)
