...
class TestJitting:
    ...
    def test_jit_circuit_with_parameters(self):
        ...
        r_var = Variable(0.5, "r")
        phi_var = Variable(0.5, "phi")
        theta_var = Variable(0.5, "theta")
        transmissivity_var = Variable(0.5, "transmissivity")

        initial_state = SqueezedVacuum(mode=0, r=r_var, phi=phi_var)
        BS_01 = BSgate(modes=(0, 1), theta=theta_var, phi=phi_var)
        BS_12 = BSgate(modes=(1, 2), theta=theta_var, phi=phi_var)
        att = Attenuator(mode=0, transmissivity=transmissivity_var)

        def evaluate_parameters(params):
            r"""
            Evaluate pre-defined circuit elements with the given parameters.
            """
            BS_01.parameters["theta"].value = params[0]
            BS_01.parameters["phi"].value = params[1]
            BS_12.parameters["theta"].value = params[2]
            BS_12.parameters["phi"].value = params[3]
            state_out = (
                initial_state
                >> initial_state.on(1)
                >> initial_state.on(2)
