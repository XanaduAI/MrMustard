...
from mrmustard.lab import DM, GaussianDM, Ggate, SqueezedVacuum, Vacuum
...
def test_compactFock_diagonal():
    ...
    A, B, G0 = GaussianDM.random([0, 1, 2]).bargmann_triple()
...
def test_compactFock_diagonal_gradients():
    ...
    """
    symplectic_var = Variable(math.random_symplectic(1), "symplectic")

    def cost_fn(symplectic_var):
        G = Ggate(0, symplectic=symplectic_var)
...
def test_compactFock_1leftover_gradients():
    ...
    """
    symplectic_var = Variable(math.random_symplectic(2), "symplectic")
