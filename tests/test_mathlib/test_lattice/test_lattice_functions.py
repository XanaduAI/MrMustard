...
from mrmustard.lab import Dgate, GaussianDM, GaussianKet, Sgate, Unitary
from mrmustard.mathlib import cython_lattice
from mrmustard.mathlib.lattice.strategies.beamsplitter import (
    apply_BS_schwinger,
    sector_idx,
    sector_u,
)
from mrmustard.mathlib.lattice.strategies.binomial import binomial, binomial_dict
...
def test_apply_bs_schwinger():
    "test that the schwinger method to apply a BS works correctly"
    G = GaussianKet.random((0, 1)).fock_array([20, 20])
    G = math.asnumpy(G)
    BS = math.beamsplitter(1.0, 1.0, (20, 20, 20, 20), method="vanilla")
    ...
    Gg = math.asnumpy(Gg)
    BS = math.beamsplitter(2.0, -1.0, (20, 20, 20, 20), method="vanilla")
...
def test_diagonalbatchNumba_vs_diagonalNumba(batch_size):
    ...
    state = GaussianDM.random((0, 1, 2)) >> Dgate(0, 0.0) >> Dgate(1, 0.1) >> Dgate(2, 0.2)
...
def test_vanillaNumba_vs_binomial():
    ...
    with settings(SEED=42):
        ...
        ket_vanilla = cython_lattice.vanilla(shape=(10, 10), A=A, b=b, c=c)[:5, :5]
...
def test_vanilla_stable():
    ...
    with settings(STABLE_FOCK_CONVERSION=True):
        assert np.allclose(
            ...
            math.displacement(alpha=4.0 + 4.0j, shape=(1000, 1000)),
        )
...
def test_batched_displacement():
    ...
    D_batched = math.displacement(alpha=alphas, shape=(5, 5))
    for i in range(len(alphas)):
        D_single = math.displacement(alpha=alphas[i], shape=(5, 5))
