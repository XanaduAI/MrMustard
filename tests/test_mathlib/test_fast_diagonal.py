...
from mrmustard.lab import Dgate, GaussianDM
from mrmustard.mathlib.lattice.strategies import fast_diagonal


def test_fast_diagonal_2modes():
    r"""Test that the fast diagonal function works for a 2-mode Gaussian state."""
    A, b, c = (
        GaussianDM.random([0, 1]) >> Dgate(0, 0.4 + 0.6j) >> Dgate(1, 0.4 + 0.6j)
    ).bargmann_triple()
