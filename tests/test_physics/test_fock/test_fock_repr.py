...
class TestFockRepresentation:
    ...
    def test_init_batched(self):
        ...
        fock = Fock(self.array1578, batch_flag=True)
    ...
    def test_init_non_batched(self):
        ...
        fock = Fock(self.array578, batch_flag=False)
        assert isinstance(fock, Fock)
    ...
    def test_init_from_ansatz_batched(self):
        ...
        fock = Fock.from_ansatz(ArrayAnsatz(array=self.array5578), batch_flag=True)
        assert isinstance(fock, Fock)
    ...
    def test_init_from_ansatz_non_batched(self):
        ...
        fock = Fock.from_ansatz(ArrayAnsatz(array=self.array578), batch_flag=False)
        assert isinstance(fock, Fock)
        assert fock.array.shape == (1, 5, 7, 8)
        assert np.allclose(fock.array[0, :, :, :], self.array578)
    ...
    def test_and(self):
        ...
        fock1 = Fock(self.array1578, batch_flag=True)
        fock2 = Fock(self.array5578, batch_flag=True)
    ...
    def test_matmul(self):
        ...
        fock1 = Fock(self.array2578, batch_flag=True)
