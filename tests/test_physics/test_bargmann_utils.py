...
from mrmustard.lab_dev import Attenuator, Dgate, GKet, Ggate
...
#     G = GKet([0,1]) >> Dgate([0,1], 0.1, 0.2)

#     cov, mean, _ = [x[0] for x in G.phase_space(s=0)]

#     for x, y in zip(G.bargmann_triple(), wigner_to_bargmann_psi(cov, mean)):
#         assert np.allclose(x, y)


def test_wigner_to_bargmann_rho():
    """Test that the Bargmann representation of a dm is correct"""
    G = GKet([0, 1]) >> Dgate([0, 1], 0.1, 0.2) >> Attenuator([0, 1], 0.9)
    cov, mean, _ = [x[0] for x in G.phase_space(s=0)]

    for x, y in zip(G.bargmann_triple(), wigner_to_bargmann_rho(cov, mean)):
        assert np.allclose(x, y)
