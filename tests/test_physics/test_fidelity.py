...
class TestCovMeansFidelity:
    ...
    def test_fidelity_is_symmetric(self, num_modes, hbar, pure, block_diag):
        ...
        with settings(HBAR=hbar):
            ...
            means1 = np.sqrt(2 * hbar) * settings.get_rng().random(2 * num_modes)
            cov2 = random_covariance(num_modes, hbar=hbar, pure=pure, block_diag=block_diag)
            means2 = np.sqrt(2 * hbar) * settings.get_rng().random(2 * num_modes)
    ...
    def test_fidelity_with_self(self, num_modes, hbar, pure, block_diag):
        ...
        with settings(HBAR=hbar):
            ...
            means = settings.get_rng().random(2 * num_modes)
    ...
    def test_fidelity_coherent_state(self, num_modes, hbar):
        ...
        with settings(HBAR=hbar):
            beta1 = settings.get_rng().random(num_modes) + 1j * settings.get_rng().random(num_modes)
            beta2 = settings.get_rng().random(num_modes) + 1j * settings.get_rng().random(num_modes)
    ...
    @pytest.mark.parametrize(
        "alpha", settings.get_rng().random(10) + 1j * settings.get_rng().random(10)
    )
