...
from mrmustard.physics.gaussian import controlled_X, controlled_Z, number_means
...
def test_amplifier_attenuator_on_coherent_coherent(eta, x, y):
    ...
    assert Vacuum([0]) >> Dgate([0], x, y) >> Amplifier([0], 1 / eta) >> Attenuator(
        [0], eta
    ) == Thermal([0], ((1 / eta) - 1) * eta) >> Dgate([0], x, y)
...
def test_number_means(x, y):
    ...
    cov, mean, _ = [x[0] for x in state.phase_space(s=0)]
    num_mean = number_means(cov, mean)[0]
    assert np.allclose(num_mean, x * x + y * y)
