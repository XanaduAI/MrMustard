...
@pytest.mark.parametrize('obj_a, obj_b', [(MockAnimal(), MockAnimal()), 
                                          (MockFruit(), MockFruit())
                                          ])
...
@pytest.mark.parametrize('obj_type_a, obj_type_b', [(MockAnimal(), MockFruit()), 
                                                    (MockFruit(), MockAnimal())
                                                    ])
...
@pytest.mark.parametrize('obj_type_a, obj_type_b, truth_val', [
                                                                (MockAnimal(), MockFruit(), False),
                                                                (MockAnimal(), MockAnimal(), True),
                                                                (MockFruit(), MockFruit(), True),
                                                                (MockFruit(), MockAnimal(), False)
                                                                ])
def test_dtc_returns_correct_bool_with_objects_of_different_or_same_type(obj_type_a, 
                                                                         obj_type_b, 
                                                                         truth_val):
