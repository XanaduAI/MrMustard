...
def test_update_symplectic(n):
    ...
    for i in range(20):
        dS_euclidean = tf.Variable(np.random.random((2*n, 2*n)) + 1j*np.random.random((2*n, 2*n)))
...
def test_update_unitary(n):
    ...
    for i in range(20):
        ...
        sym = np.block([[np.real(U.numpy()),-np.imag(U.numpy())],[np.imag(U.numpy()),np.real(U.numpy())]])
