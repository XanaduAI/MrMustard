...
class TestConverter:
    ...
    def test_convert_from_wignerdm_to_bargmanndm_squeezed_state(self, r, phi):
        ...
        assert np.allclose(
            bargmann_dm.A,
            math.array(
                [[math.tanh(r) * math.exp(-1j * phi), 0], [0, math.tanh(r) * math.exp(1j * phi)]]
            ),
        )
    ...
    def test_convert_from_wignerket_to_fockket_to_wavefunctionqket_coherent_state(self, x, y):
        ...
        wigner_ket = WignerKet(symplectic=symplectic, displacement=displacement)
        qs = [0, 0.5, 1.7]
        wavefunctionq_ket = self.converter.convert(
            source=wigner_ket, destination="WaveFunction", qs=qs
        )

        def coherent_state_analytic(x, alpha):
            x = x * np.sqrt(1 / settings.HBAR)
            return (
                np.exp(-math.norm(alpha) ** 2 / 2)
                / (np.pi * settings.HBAR) ** (1 / 4)
                * np.exp(x**2 / 2)
                * np.exp(-((x - alpha / np.sqrt(2)) ** 2))
            )
