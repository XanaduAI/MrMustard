...
"""Tests for the ``ComputationalGraph`` class."""
...
from mrmustard.lab_dev.computational_graphs.computational_graph import ComputationalGraph
...
class TestComputationalGraph:
    ...
    def test_init(self):
        ...
        circ1 = ComputationalGraph([vac, s01, bs01, bs12])
        assert circ1.components == [vac, s01, bs01, bs12]
        assert circ1.path == [(0, 1), (0, 2), (0, 3)]

        circ2 = ComputationalGraph() >> vac >> s01 >> bs01 >> bs12
    ...
    def test_propagate_shapes(self):
        ...
        circ = ComputationalGraph([Coherent([0], x=1.0), Dgate([0], 0.1)])
        ...
        circ = ComputationalGraph([SqueezedVacuum([0, 1], r=[0.5, -0.5]), BSgate((0, 1), 0.9)])
    ...
    def test_lookup_path(self, capfd):
        ...
        circ = ComputationalGraph([vac, s01, bs01, bs12])
    ...
    def test_path(self, path):
        ...
        graph = ComputationalGraph([vac12, d1, d2, d12, a1, n12])
        graph.path = path

        assert graph.path == path

    def test_path_errors(self):
        vac12 = Vacuum([1, 2])

        with pytest.raises(ValueError, match="Overlapping"):
            ComputationalGraph([vac12, vac12])

        with pytest.raises(ValueError, match="Overlapping"):
            ComputationalGraph([vac12.adjoint, vac12.adjoint])
    ...
    def test_eq(self):
        ...
        assert ComputationalGraph([vac, s01]) == ComputationalGraph([vac, s01])
        assert ComputationalGraph([vac, s01]) != ComputationalGraph([vac, s01, bs01])

    def test_len(self):
        vac = Vacuum([0, 1, 2])
        s01 = Sgate([0, 1])

        assert len(ComputationalGraph([vac])) == 1
        assert len(ComputationalGraph([vac, s01])) == 2
        assert len(ComputationalGraph([vac, s01, s01])) == 3
    ...
    def test_rshift(self):
        ...
        circ1 = ComputationalGraph([vac]) >> s01
        circ2 = ComputationalGraph([bs01, bs12])

        assert circ1 >> circ2 == ComputationalGraph([vac, s01, bs01, bs12])
    ...
    def test_repr(self):
        ...
        n12 = Number([0, 1], n=3)
        n2 = Number([2], n=3)
        cc = GraphComponent(bs01.representation, "my_cc")

        assert repr(ComputationalGraph()) == ""

        circ1 = ComputationalGraph([vac012])
        r1 = ""
        r1 += "\nmode 0:     ◖Vac◗"
        r1 += "\nmode 1:     ◖Vac◗"
        r1 += "\nmode 2:     ◖Vac◗"
        ...
        circ2 = ComputationalGraph([vac012, s01, bs01, bs12, cc, n12.dual])
        r2 = ""
        ...
        circ4 = ComputationalGraph([vac01, s01, vac2, bs01, bs12, n2.dual, cc, n12.dual])
        r4 = ""
        r4 += "\nmode 0:     ◖Vac◗──S(0.0,2.0)──╭•──────────────────────────CC─────────|3)=(3,3)"
        r4 += "\nmode 1:     ◖Vac◗──S(1.0,3.0)──╰BS(0.0,0.0)──╭•────────────CC─────────|3)=(3,3)"
        ...
        circ5 = ComputationalGraph() >> vac1 >> bs01 >> vac1.dual >> vac1 >> bs01 >> vac1.dual
        r5 = ""
        r5 += "\nmode 0:          ──╭•───────────────────────────╭•──────────────────"
    ...
    def test_repr_issue_334(self):
        ...
        """
        circ1 = ComputationalGraph([Sgate([0, 1], [1.0, -1.0], [2.0, -2.0])])
        r1 = ""
        r1 += "\nmode 0:   ──S(1.0,2.0)──"
        r1 += "\nmode 1:   ──S(-1.0,-2.0)"
        r1 += "\n\n"
    ...
    def test_optimize_path(self):
        ...
        circ = ComputationalGraph([Number([0], n=15), Sgate([0], r=1.0), Coherent([0], x=1.0).dual])
    ...
    def test_wrong_path(self):
        ...
        circ = ComputationalGraph([Number([0], n=15), Sgate([0], r=1.0), Dgate([0], x=1.0)])
        circ.path = [(0, 3)]
    ...
    def test_path_is_loaded(self, tmpdir):
        ...
        circ = ComputationalGraph([vac, S0, s0, bs01, c0, c1])
        base_path = circ.path
